================================================================================
 smf80 - Estrazione, filtro e dump dei record SMF 80/81 (RACF) per z/OS
================================================================================
...
I due programmi principali sono:
  ...
  smf80dmp.c
      Filtra i record SMF 80 in base a un file di criteri e ne produce una
      stampa esadecimale formattata (dump), comprensiva di header e delle
      varie sezioni relocabili DTA e DT2.

  smf80ext.c
      Filtra i record SMF 80 in base a un file di criteri e ne crea un
      estratto (in formato originale) rispondente ai filtri forniti, da
      utilizzare per ulteriori elaborazioni a valle.
...
--------------------------------------------------------------------------------
Il file di criteri e' un file sequenziale in cui ogni riga ha la forma:

    <nome car> <op> <lista valori> [ + ]
      [ continuazione lista valori ]

dove:
  <nome car>   e' uno dei campi del record SMF 80 riconosciuti:
                 ...
                 AUTH    Autorita' usate      (SMF80ATH)
                 RES     Resource name        (rel. sect. 01)
                 USRJES  Resource Owner JESSPOOL (rel. sect. 01)
  ...
  <op>         puo' essere:
                 ...
                 ABR  abbreviato, uno dei valori nella lista e'
                      abbreviazione del valore nel record

  <lista valori>
               elenco dei valori da ricercare; la lista puo' continuare su
               piu' righe terminando la riga con "+". La lunghezza massima
               di ogni valore e' 44 caratteri.

Tutti i criteri presenti nel file sono posti in AND tra loro: un record e'
selezionato solo se soddisfa tutti i criteri.

--------------------------------------------------------------------------------
DD UTILIZZATE
--------------------------------------------------------------------------------
  DD:UTIPARM   (input)  file sequenziale con i criteri di estrazione
  DD:UTI001    (input)  file sequenziale VBS (o concatenazione) con i
                        record SMF da filtrare
  DD:UTICNTL   (input)  PDS FB 80 con eventi ed event qualifier per la
                        verifica e la decodifica
  DD:UTI002    (output) file sequenziale VBA con l'esito del filtro
                        (dump esadecimale per smf80dmp, estratto per
                        smf80ext)

Autorizzazioni richieste: READ sui dataset SMF di input, ALTER/UPDATE sul
dataset di output (UTI002).
...
Programmi principali:
  ...
  smf81dec.c      decodifica dei record SMF 81 (inizializzazione RACF),
                  richiamato da smf80ext
...
Funzioni di libreria (raccolte nella libreria myext):
  ...
  createDta.c     costruttore delle strutture per le sezioni relocabili
                  DTA e DT2 del record SMF 80
  creDTA_2.c      costruttore per la struttura dati sezioni relocabili
                  (normali ed extended)
...
COMPILAZIONE
--------------------------------------------------------------------------------
La compilazione e' pensata per l'ambiente z/OS UNIX System Services, con il
compilatore xlc e il tool make (nmake-like). I makefile di riferimento sono
smf80dmp.mk e smf80ext.mk: compilano le funzioni di supporto in una libreria
statica (myext.a) e poi il programma principale, producendo i moduli
eseguibili e i relativi listati di compilazione (.lst).
...
LICENZA
--------------------------------------------------------------------------------
Questo progetto e' distribuito secondo i termini della licenza indicata nel
